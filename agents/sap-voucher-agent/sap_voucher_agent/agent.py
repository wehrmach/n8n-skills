...
SYSTEM_PROMPT = """\
당신은 한국 기업의 회계 증빙을 SAP ERP 에 전기하는 AI 회계 에이전트다.
표준 BAPI 만 사용하며, 절차를 건너뛰지 않는다.

[작업 절차 - 증빙 1건마다 순서대로]
1. extract_voucher 로 증빙을 판독한다.
2. resolve_master 로 거래처를 SAP 코드에 매핑한다. 미등록이면 사용자에게 보고한다.
3. plan_posting 으로 BAPI 전기 계획을 만든다. 계획을 사용자에게 설명한다.
4. simulate_posting 으로 반드시 DRY-RUN 검증을 먼저 수행한다.
5. 검증을 통과하고 승인 요건이 충족되면 execute_posting 으로 전기한다.
6. 전기 결과(문서번호)를 보고한다.
...
[보고 형식]
한국어로 간결하게. 증빙별로 '유형 / 금액 / 사용 BAPI / 결과(문서번호 또는 사유)'를
표로 정리하고, 사람이 처리해야 할 항목을 마지막에 목록으로 남긴다."""
        ...
        def posting_report() -> str:
            ...
            for vid, doc in s.vouchers.items():
                ...
                rows.append(f"| {vid} | {KR_NAME[doc.doc_type]} | "
                            f"{_money(doc.gross_total)} | {bapis} | {res} |")
            return "\n".join(rows)

        return [list_voucher_files, extract_voucher, resolve_master,
                explain_bapi_route, patch_voucher, plan_posting,
                inspect_bapi_params, simulate_posting, execute_posting,
                posting_report]
